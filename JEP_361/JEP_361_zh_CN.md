...
`switch` 表达式的 case 必须可穷举； 对于所有可能的值，必须有一个匹配的 switch 标签。 （显然，`switch` 语句则不需要详尽无遗。）

实际上，这通常意味着需要一个 `default` 子句； 但是，对于涵盖所有已知常量的 `enum` `switch` 表达式，编译器会插入一个 `default` 子句以指示 `enum` 定义在编译时和运行时之间发生了变化。 依靠这种隐式的 `default` 子句插入，可以使代码更健壮； 现在，当重新编译代码时，编译器会检查所有情况是否都已明确处理。 如果开发人员插入了一个明确的 `default` 子句，一个可能的错误就会被隐藏起来（就像下面的情况一样）。
...
这有很多后果。 首先，编译器检查每个 switch 标签，如果匹配则可以产生一个值。
...
进一步的后果, 是 `switch` 表达式中的控制语句: `break`, `yield`, `return` 和 `continue` 不能再跳转，如下所示：
...
是否需要带有 `case L ->` 标签的 `switch` 语句, 有时候并不是特别清晰。 下面是将其包括在内的考虑点：

- 有通过副作用进行操作的 `switch` 语句，但通常仍然是 “每个标签一个动作”。 将这些与新式标签一起使用, 可以使语句更加直接且不易出错。
- `switch` 语句块中的 default 控制流是通过滑落（fall through）, 而不是突破(break out)，这在 Java 语言的早期是一个不幸的选择， 并且仍然是开发人员的一个大麻烦。 通过为一般的 `switch` 构造解决这个问题，而不仅仅是针对 `switch` 表达式，可以减少这种选择的影响。
- 通过将预期的好处梳理成正交特征（比如: 表达性、更好的控制流、更健全的作用域范围），`switch`表达式和 `switch` 语句可以有更多共同点。 `switch` 表达式和 `switch` 语句之间的差异越大，学习语言就越复杂，开发人员需要掌握的那些本不重要的小细节就越多。